...
CAP::TableParser::TableParser()
:
FileParser()
{  }

CAP::TableParser::TableParser(const TableParser & parser)
:
FileParser(parser)
{  }

CAP::TableParser & CAP::TableParser::operator=(const CAP::TableParser & parser)
{
  if (this!=&parser)
    {
...
void CAP::TableParser::read(const String  & _inputPath,
...
//!
vector< vector<double> > CAP::TableParser::readBlockData(istream &inputStream)
{
  ...
  if (rowSize==0) throw FileException("lineSize==0","CAP::TableParser::readBlockData(istream &inputStream)");
  data.push_back(row);
  // rest of the lines:
  while (inputStream.eof()==false)
    ...
    inputStream.getline(buffer,99999);
    row = stringToDoubles(buffer);
    if (rowSize != row.size()) throw FileException("rowSize != row.size()","CAP::TableParser::readBlockData(istream &inputStream)");
    data.push_back(row);
    }
  return data;
}
