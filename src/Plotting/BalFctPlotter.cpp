...
bf_General_Titles(),
bf_DeltaYDeltaPhi_Titles(),
bf_DeltaY_Titles(),
bf_DeltaPhi_Titles(),
bf_Integral_DeltaY_Titles(),
bf_IntegralSum_DeltaY_Titles(),
bf_Width_DeltaY_Titles(),
bf_Width_DeltaPhi_Titles(),
bf_IntegralSumVsName_Titles(),

deltaY_Name("DeltaY"),
deltaPhi_Name("#Delta y"),
...
void BalFctPlotter::loadHistograms()
{
  ...
  if (reportInfo(__FUNCTION__))
    {
    cout << endl;
    printValue("bf_InputFiles.size()",nFiles);
    }

  for (Size_t iFile=0; iFile<nFiles; iFile++)
    {
    Size_t nPairs = pairNames.size();
    printValue("nPairs",nPairs);
    for (Size_t iPair=0; iPair<nPairs; iPair++)
      {
      printValue("iPair",iPair);
      printValue("pairName", pairNames[iPair]);
      printValue("pairTitle",pairTitles[iPair]);
      Size_t index = iFile*pairNames.size() + iPair;
      printValue("bf_DeltaYDeltaPhi_HistoName",bf_DeltaYDeltaPhi_HistoNames[index]);
      printValue("bf_DeltaY_HistoNames",bf_DeltaY_HistoNames[index]);
      printValue("bf_DeltaPhi_HistoNames",bf_DeltaPhi_HistoNames[index]);
   ...
   for (Size_t iFile=0; iFile<nFiles; iFile++)
    ...
    for (Size_t iTrigger=0; iTrigger<nSpecies; iTrigger++)
      ...
      for (Size_t iAssoc=0; iAssoc<nSpecies; iAssoc++)
        ...
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("xLeft", 0.0);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("xRight",0.0);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("yLow",  0.0);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("yHigh", 0.0);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("useLegend",false);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("useTitles",false);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("useLabels",true);
        bf_DeltaYDeltaPhi_LegendConfig.addProperty("useNDC",false); // use NDC or PAD coordinates
...
//      printValue("minima.size()",minima.size());
//      printValue("maxima.size()",maxima.size());
      ...
      bf_DeltaY_LegendConfig.addProperty("xLeft", x);
      bf_DeltaY_LegendConfig.addProperty("xRight",x+0.2*xRange);
      bf_DeltaY_LegendConfig.addProperty("yLow",  yMax - 0.7*yRange);
      bf_DeltaY_LegendConfig.addProperty("yHigh", yMax - 0.3*yRange);
      bf_DeltaY_LegendConfig.addProperty("useLegend",true);
      bf_DeltaY_LegendConfig.addProperty("useTitles",false);
      bf_DeltaY_LegendConfig.addProperty("useLabels",true);
      bf_DeltaY_LegendConfig.addProperty("textSize", 0.065);
      if (histos.size()<5)
        bf_DeltaY_LegendConfig.addProperty("nColumns",     1);
      else
        bf_DeltaY_LegendConfig.addProperty("nColumns",     2);
      String canvasName = createName(outputFileNameBase,bf_CanvasNameBases[0],bf_TypeNames[bf_Types[0]],pairNames[iTrigger*nSpecies+iAssoc],deltaY_Name);
      plot(canvasName,
...
//        histoRatios.push_back(h);
    ...
    bf_DeltaY_LegendConfig.addProperty("xLeft", x);
    bf_DeltaY_LegendConfig.addProperty("xRight",x+0.3*xRange);
    bf_DeltaY_LegendConfig.addProperty("yLow",  yMax - 0.7*yRange);
    bf_DeltaY_LegendConfig.addProperty("yHigh", yMax - 0.2*yRange);
    bf_DeltaY_LegendConfig.addProperty("useLegend",true);
    bf_DeltaY_LegendConfig.addProperty("useTitles",false);
    bf_DeltaY_LegendConfig.addProperty("useLabels",true);
    bf_DeltaY_LegendConfig.addProperty("useNDC",false);
    bf_DeltaY_LegendConfig.addProperty("textColor",    1);
    bf_DeltaY_LegendConfig.addProperty("textSize",     0.06);
    if (histos.size()<5)
      bf_DeltaY_LegendConfig.addProperty("nColumns",     1);
    else
      bf_DeltaY_LegendConfig.addProperty("nColumns",     2);

    String canvasName = createName(outputFileNameBase,
                                   bf_CanvasNameBases[0],
                                   bf_TypeNames[bf_Types[0]],
                                   speciesNames[iTrigger],
                                   deltaY_Name);
    plot(canvasName,
         bf_DeltaY_CanvasConfig,
         bf_DeltaY_GraphConfigs,
...
void BalFctPlotter::plotBfIntegralVsPairvsDeltaY()
{
  ...
  for (Size_t iTrigger=0; iTrigger<nSpecies; iTrigger++)
    ...
    printValue("graphs.size()",graphs.size());
    ...
    if (false)
      ...
      bf_Integral_DeltaY_LegendConfig.addProperty("xLeft", x);
      bf_Integral_DeltaY_LegendConfig.addProperty("xRight",x+0.15*xRange);
      bf_Integral_DeltaY_LegendConfig.addProperty("yLow",  yMax - 0.5*yRange);
      bf_Integral_DeltaY_LegendConfig.addProperty("yHigh", yMax - 0.2*yRange);
      bf_Integral_DeltaY_LegendConfig.addProperty("useLegend",true);
      bf_Integral_DeltaY_LegendConfig.addProperty("useTitles",false);
      bf_Integral_DeltaY_LegendConfig.addProperty("useLabels",true);
      bf_Integral_DeltaY_LegendConfig.addProperty("useNDC",false);
      bf_Integral_DeltaY_LegendConfig.addProperty("textColor",    1);
      bf_Integral_DeltaY_LegendConfig.addProperty("textSize",     0.05);
      bf_Integral_DeltaY_LegendConfig.addProperty("nColumns",     1);
      }
    else
      {
      x = 0.25;
      ...
      yRange = 0.3;
      bf_Integral_DeltaY_LegendConfig.addLabel(bf_General_Titles[0],x,y,0.0,1,0.06);
      bf_Integral_DeltaY_LegendConfig.addProperty("xLeft", 0.25);
      bf_Integral_DeltaY_LegendConfig.addProperty("xRight",1.20);
      bf_Integral_DeltaY_LegendConfig.addProperty("yLow",  0.60);
      bf_Integral_DeltaY_LegendConfig.addProperty("yHigh", 1.250);
      bf_Integral_DeltaY_LegendConfig.addProperty("useLegend",true);
      bf_Integral_DeltaY_LegendConfig.addProperty("useTitles",false);
      bf_Integral_DeltaY_LegendConfig.addProperty("useLabels",true);
      bf_Integral_DeltaY_LegendConfig.addProperty("useNDC",false);
      bf_Integral_DeltaY_LegendConfig.addProperty("textColor",    1);
      bf_Integral_DeltaY_LegendConfig.addProperty("textSize",     0.055);
      bf_Integral_DeltaY_LegendConfig.addProperty("nColumns",     3);
      }

    String canvasName = createName(outputFileNameBase,
                                   bf_CanvasNameBases[0],
                                   bf_IntegralTypeNames[bf_Types[0]],
                                   speciesNames[iTrigger],
                                   deltaY_Name);
    plot(canvasName,
...
//      printValue("bf_Title",bf_Title);
//      printValue("pre-tweak - bf_min",bf_min);
//      printValue("pre-tweak - bf_max",bf_max);

      tweak(bf_min,bf_max);
//      printValue("post-tweak - bf_min",bf_min);
//      printValue("post-tweak - bf_max",bf_max);
      ...
      bf_Integral_DeltaY_LegendConfig.addProperty("xLeft", 0.25);
      bf_Integral_DeltaY_LegendConfig.addProperty("xRight",0.65);
//      bf_Integral_DeltaY_LegendConfig.addProperty("xLeft", x);
//      bf_Integral_DeltaY_LegendConfig.addProperty("xRight",x+0.12*xRange);
      bf_Integral_DeltaY_LegendConfig.addProperty("yLow",  yMax - 0.55*yRange);
      bf_Integral_DeltaY_LegendConfig.addProperty("yHigh", yMax - 0.25*yRange);
//      bf_Integral_DeltaY_LegendConfig.addProperty("yLow",  yMax + 0.015*yRange);
//      bf_Integral_DeltaY_LegendConfig.addProperty("yHigh", yMax + 0.35*yRange);
      bf_Integral_DeltaY_LegendConfig.addProperty("useLegend",true);
      bf_Integral_DeltaY_LegendConfig.addProperty("useTitles",false);
      bf_Integral_DeltaY_LegendConfig.addProperty("useLabels",true);
      bf_Integral_DeltaY_LegendConfig.addProperty("useNDC",false);
      bf_Integral_DeltaY_LegendConfig.addProperty("textColor",    1);
      bf_Integral_DeltaY_LegendConfig.addProperty("textSize",     0.06);
      bf_Integral_DeltaY_LegendConfig.addProperty("nColumns",     1);

      String canvasName = createName(outputFileNameBase,
                                     bf_CanvasNameBases[0],
                                     bf_IntegralTypeNames[bf_Types[0]],
                                     pairNames[iTrigger*nSpecies+iAssoc],
                                     deltaY_Name);
      plot(canvasName,
           bf_Integral_DeltaY_CanvasConfig,
           bf_DeltaY_GraphConfigs,
...
void BalFctPlotter::plotBFIntegralSumvsPairvsDeltaY()
{
  ...
  for (Size_t iTrigger=0; iTrigger<nSpecies; iTrigger++)
    ...
    x = 0.20;
    bf_Integral_DeltaY_LegendConfig.addProperty("xLeft", x);
    bf_Integral_DeltaY_LegendConfig.addProperty("xRight",x+0.95*xRange);
//    bf_Integral_DeltaY_LegendConfig.addProperty("yLow",  yMax - 0.5*yRange);
//    bf_Integral_DeltaY_LegendConfig.addProperty("yHigh", yMax - 0.2*yRange);
    bf_Integral_DeltaY_LegendConfig.addProperty("yLow",  yMax + 0.01*yRange);
    bf_Integral_DeltaY_LegendConfig.addProperty("yHigh", yMax + 0.3*yRange);
    bf_Integral_DeltaY_LegendConfig.addProperty("useLegend",true);
    bf_Integral_DeltaY_LegendConfig.addProperty("useTitles",false);
    bf_Integral_DeltaY_LegendConfig.addProperty("useLabels",true);
    bf_Integral_DeltaY_LegendConfig.addProperty("useNDC",false);
...
void BalFctPlotter::execute()
{
  if (reportInfo(__FUNCTION__))
    {
    cout << endl;
    printValue("rangeOption",rangeOption);
    //printValue("correlationOption",correlationOption);
    printValue("speciesOption",speciesOption);
    printValue("rapidityOption",rapidityOption);
    }
  ...
  if (reportInfo(__FUNCTION__)) printValue("doPlotsBfVsDeltaYDeltaPhi",doPlotsBfVsDeltaYDeltaPhi);
  if (doPlotsBfVsDeltaYDeltaPhi) plotBF2D();

  if (reportInfo(__FUNCTION__)) printValue("doPlotsBfVsDeltaY",doPlotsBfVsDeltaY);
  ...
  if (reportInfo(__FUNCTION__)) printValue("doPlotsBfVsDeltaPhi",doPlotsBfVsDeltaPhi);
  if (doPlotsBfVsDeltaPhi) plotBfVsPairvsDeltaPhi();
  if (reportInfo(__FUNCTION__)) printValue("doPlotsBfIntegralVsDeltaY",doPlotsBfIntegralVsDeltaY);
  ...
  if (doPrint)
    printAllCanvas(outputPathNameBase,doPrintGif,doPrintPdf,doPrintSvg,doPrintPng,doPrintC);
}

void BalFctPlotter::setSpeciesArrays()
{
  if (reportInfo(__FUNCTION__)) printValue("SpeciesOption",speciesOption);
  speciesNames.clear();
  speciesTitles.clear();
  speciesPath = "";
...
void BalFctPlotter::createNames()
{
  ...
  for (Size_t iFile=0; iFile<nFiles; iFile++)
    ...
    printValue("baseHistoName",baseHistoName);
    printValue("baseHistoTitle",baseHistoTitle);
    printValue("baseCanvasName",baseCanvasName);
    printValue("correlatorSourceName",correlatorSourceName);
    ...
    for (Size_t iPair=0; iPair<pairNames.size(); iPair++)
      ...
      printValue("pair index",iPair);
      printValue("pairName",pairName);
      printValue("pairTitle",pairTitle);
      printValue("bf_DeltaYDeltaPhi_InputName",bf_DeltaYDeltaPhi_InputName);
      printValue("bf_DeltaY_InputName",bf_DeltaY_InputName);
      printValue("bf_DeltaPhi_InputName",bf_DeltaPhi_InputName);
      printValue("bf_DeltaYDeltaPhi_Title",title);
      printValue("bf_DeltaY_Titles",title);
      printValue("bf_DeltaPhi_Title",title);
      printValue("bf_Integral_DeltaY_Title",title);
      printValue("bf_IntegralSum_DeltaY_Title",title);
      }
    }
}
...
void BalFctPlotter::openInputFiles()
{
  ...
  if (reportInfo(__FUNCTION__)) printValue("nFiles",nFiles);
  for (Size_t iFile=0; iFile<nFiles; iFile++)
    {
    ...
    if (reportInfo(__FUNCTION__))
      ...
      cout << endl;
      printValue("iFile",iFile);
      printValue("Input path",bf_InputPathNames[iFile]);
      printValue("File name",bf_InputFileNames[iFile]);
      printValue("Title base",bf_InputFileTitles[iFile]);
      printValue("Histo Name Base",bf_HistoNameBases[iFile]);
      printValue("Histo Title Base",bf_HistoTitleBases[iFile]);
      printValue("Canvas Name Base",bf_CanvasNameBases[iFile]);
      }
    }
}
} // namespace CAP
