#ifdef __CINT__

#pragma link off all globals;
#pragma link off all classes;
#pragma link off all functions;
#pragma link C++ class CAP::RootChainManager+;
#pragma link C++ class CAP::Event+;
#pragma link C++ class CAP::EventCountHistos+;
#pragma link C++ class CAP::EventFilter+;
#pragma link C++ class CAP::EventAccountant+;
#pragma link C++ class CAP::EventTask+;
#pragma link C++ class CAP::EventIterator+;
#pragma link C++ class CAP::Particle+;
#pragma link C++ class CAP::ParticleDecayer+;
#pragma link C++ class CAP::ParticleDecayerTask+;
#pragma link C++ class CAP::ParticleFilter+;
#pragma link C++ class CAP::ParticlePairFilter+;
#pragma link C++ class CAP::JetFilter+;
#pragma link C++ class CAP::EfficiencyFilter+;
#pragma link C++ class CAP::FilterCreator+;
#pragma link C++ class CAP::Manager<CAP::Event>+;
#pragma link C++ class CAP::Manager<CAP::EventFilter>+;
#pragma link C++ class CAP::Manager<CAP::ParticleFilter>+;
#pragma link C++ class CAP::Manager<CAP::EfficiencyFilter>+;
#pragma link C++ class CAP::Manager<CAP::JetFilter>+;
#pragma link C++ class CAP::Filter<CAP::Event>+;
#pragma link C++ class CAP::Filter<CAP::Particle>+;
#pragma link C++ class CAP::Filter<fastjet::PseudoJet>+;
#pragma link C++ class CAP::Factory<CAP::Particle>+;

#endif
