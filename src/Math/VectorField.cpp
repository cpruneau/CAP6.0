...
ClassImp(CAP::VectorField);

using CAP::VectorField;
using CAP::NamedObject;
...
:
NamedObject("VectorField","Field",1.0,""),
constValue(0.0),
...
VectorField::VectorField(const String & aName, double constValue)
:
NamedObject(aName,aName,1.0,""),
...
VectorField::VectorField(const String & aName,
...
type(1)
{
  ...
  if (mXampx<=mXmin || mYampx<=mYmin || mZampx<=mZmin)
    {
    throw MathException("Invalid input: mXampx<=mXmin || mYampx<=mYmin || mZampx<=mZmin",__FUNCTION__);
    }
...
void VectorField::setValue(const String & aName,
                           double aXmin, double aXampx, unsigned int aXpts,
                           double aYmin, double aYampx, unsigned int aYpts,
                           double aZmin, double aZampx, unsigned int aZpts,
                           double initValue)
