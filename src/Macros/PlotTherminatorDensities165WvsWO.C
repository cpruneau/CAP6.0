...
int PlotTherminatorDensities165WvsWO()
{
  ...
  vector<GraphConfiguration*>  graphConfigurations1D = GraphConfiguration::createConfigurationPalette(14,1);
  //vector<GraphConfiguration*>  graphConfigurations2D = GraphConfiguration::createConfigurationPalette(12,2);
  //landscapeLinear.addParameter("windowHeight", 300);
  for (unsigned int iGraph=0; iGraph<14; iGraph++)
    {
    ...
    graphConfigurations1D[iGraph]->addParameter("markerSize",   0.6);
    graphConfigurations1D[iGraph]->addParameter("markerColor", kBlack);
    graphConfigurations1D[iGraph]->addParameter("lineColor",   kBlack);
    ...
    graphConfigurations1D[0]->addParameter("markerStyle", kFullSquare);

  graphConfigurations1D[1]->addParameter("markerColor", kRed);
  graphConfigurations1D[1]->addParameter("lineColor",   kRed);
  graphConfigurations1D[1]->addParameter("markerStyle", kFullSquare);

  graphConfigurations1D[2]->addParameter("markerColor", kGreen);
  graphConfigurations1D[2]->addParameter("lineColor",   kGreen);
  graphConfigurations1D[2]->addParameter("markerStyle", kFullSquare);

  graphConfigurations1D[3]->addParameter("markerColor", kBlue);
  graphConfigurations1D[3]->addParameter("lineColor",   kBlue);
  graphConfigurations1D[3]->addParameter("markerStyle", kFullSquare);

  graphConfigurations1D[4]->addParameter("markerColor", 6);
  graphConfigurations1D[4]->addParameter("lineColor",   6);
  graphConfigurations1D[4]->addParameter("markerStyle", kFullSquare);


    graphConfigurations1D[5]->addParameter("markerColor", kBlack);
    graphConfigurations1D[5]->addParameter("lineColor",   kBlack);
    graphConfigurations1D[5]->addParameter("markerStyle", kOpenSquare);


    graphConfigurations1D[6]->addParameter("markerColor", kRed);
    graphConfigurations1D[6]->addParameter("lineColor",   kRed);
    graphConfigurations1D[6]->addParameter("markerStyle", kOpenSquare);


    graphConfigurations1D[7]->addParameter("markerColor", kGreen);
    graphConfigurations1D[7]->addParameter("lineColor",   kGreen);
    graphConfigurations1D[7]->addParameter("markerStyle", kOpenSquare);


    graphConfigurations1D[8]->addParameter("markerColor", kBlue);
    graphConfigurations1D[8]->addParameter("lineColor",   kBlue);
    graphConfigurations1D[8]->addParameter("markerStyle", kOpenSquare);


    graphConfigurations1D[9]->addParameter("markerColor", 6);
    graphConfigurations1D[9]->addParameter("lineColor",   6);
    graphConfigurations1D[9]->addParameter("markerStyle", kOpenSquare);

    graphConfigurations1D[10]->addParameter("markerColor", 7);
    graphConfigurations1D[10]->addParameter("lineColor",   7);
    graphConfigurations1D[10]->addParameter("markerStyle", kFullSquare);

    graphConfigurations1D[11]->addParameter("markerColor", 7);
    graphConfigurations1D[11]->addParameter("lineColor",   7);
    graphConfigurations1D[11]->addParameter("markerStyle", kOpenSquare);



  TString inputPath1  = "/Volumes/ClaudeDisc4/OutputFiles/Therminator/BW/T165.6/";
  TString inputPath2  = "/Volumes/ClaudeDisc4/OutputFiles/Therminator/BW/T165.6/";
  TString outputPath  = "/Volumes/ClaudeDisc4/OutputFiles/Therminator/BW/T165WvsWO/";

  //vector<TH2*>    histos2D;
  vector<TH1*>    histosPtWO;
  vector<TH1*>    histosPtW;
  //vector<TH1*>    histosY;
  vector<TString> titlesWO;
  vector<TString> titlesW;
  ...
  TFile & file2 =  plotter->openRootFile(inputPath2,"SingleGenWDecays.root","OLD");
  ...
  for (int k=0; k<nTypes; k++)
    ...
    histosPtWO.push_back(h1);
    titlesWO.push_back(particleName+" W/O");
    histosPtW.push_back(h2);
    titlesW.push_back(particleName+" W decays");
    }
  ...
  for (int iSet=40;iSet<76;iSet++)
    ...
    for (int iHisto=0;iHisto<5;iHisto++)
      {
      int index = iSet*5 + iHisto;
      histosSet.push_back(histosPtWO[index]);
      titlesSet.push_back(titlesWO[index]);
      }
    for (int iHisto=0;iHisto<5;iHisto++)
      {
      int index = iSet*5 + iHisto;
      histosSet.push_back(histosPtW[index]);
      titlesSet.push_back(titlesW[index]);
      }
