...
int PlotTherminatorDensities()
{
  ...
  TFile & file =  plotter->openRootFile(inputPath,"SingleGen.root","OLD");
  ...
  for (int k=0; k<nTypes; k++)
    ...
    if (!h)
    {
    cout << " Histograms not found: " << histoName <<   endl;
    return -1;
    }
    histosPt.push_back(h);
    titles.push_back(particleName);
    }

  TString plotNameBase = "TherminatorDensities_";
  TString plotName;
  ...
  for (int iSet=0;iSet<38;iSet++)
    {
    plotName = plotNameBase;
    plotName += iSet;
    histosSet.clear();
    titlesSet.clear();
    for (int iHisto=0;iHisto<10;iHisto++)
      {
      int index = iSet*10 + iHisto;
      histosSet.push_back(histosPt[index]);
      titlesSet.push_back(titles[index]);
      }
    plotter->plot(histosSet,graphConfigurations1D,titlesSet,plotName,landscapeLogY,TString("pT"), 0.0, 4.0,TString("#rho_{1}(pT)"),1.0E-7, 1.0E3,
