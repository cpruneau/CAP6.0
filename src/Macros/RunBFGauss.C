...
int RunBFGauss(TString configFile="Gauss/RunAna3D.ini",
               TString histogramPath="GaussTest/",
               int nLevels = 0,
               long seed=12312231)
{
  ...
  creatorTask->setTargetTaskName("RunBf");
  ...
  delete creatorTask;
  CAP::printLine();
  CAP::printString("RunBf completed successfully");
  CAP::printLine();
  return 0;
  }
...
void loadPair3D(const TString & includeBasePath)
{
  TString includePath = includeBasePath + "/ParticlePair3D/";
  gSystem->Load(includePath+"ParticlePair3DAnalyzer.hpp");
  gSystem->Load(includePath+"ParticlePair3DCalculator.hpp");
  gSystem->Load(includePath+"ParticlePair3DBfCalculator.hpp");
  gSystem->Load(includePath+"ParticlePair3DHistos.hpp");
  gSystem->Load(includePath+"ParticlePair3DDerivedHistos.hpp");
  gSystem->Load(includePath+"ParticlePair3DBfHistos.hpp");
  gSystem->Load("libParticlePair3D.dylib");
}
