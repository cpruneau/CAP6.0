...
class Analysis
{
	public:
		...
		void Generator_and_Analysis();
		void Clustering(vector<PseudoJet> &in);

		//void Clustering(vector <PseudoJet> &in);
		void Z_Analysis(std::shared_ptr<ClusterSequence> CS);
...
Analysis::Analysis(int N) {
	no_of_events = N;
	int h_bincount = 200;

	output = new TFile("Analysis Normalised version for 5,00,000.root", "recreate");
...
void Analysis :: Generator_and_Analysis(){
    ...
    for (int i = 0; i < no_of_events; i++) {
        ...
        Z_Analysis(clusterSequencePtr);
       
    }
    Renormalization();
}

void Analysis:: Store(Event &event_array){
...
//  myOtherNames.clear();
 ...
 void Analysis::Clustering(vector<PseudoJet> &input){
...
/* This function does analysis with z distribution within the jets */
void Analysis :: Z_Analysis(std::shared_ptr<ClusterSequence> CS){
...
int main()
{
	...
	Analysis Jets (500000);
	Jets.Generator_and_Analysis();
...
void mainExample()
{
  vector<SingleParticleJetAnalysis*> jetHistograms;
