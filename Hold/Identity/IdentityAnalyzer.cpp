...
void IdentityAnalyzer::initializeHistogramTask()
{
//  HistogramTask.addSet("single");
//  HistogramTask.addSet("pair");
//  HistogramTask.addSet("singleDerived");
//  HistogramTask.addSet("pairDerived");
}
...
void IdentityAnalyzer::createHistograms()
{
  ...
  for (int iEventFilter=0; iEventFilter<nEventFilters; iEventFilter++ )
    ...
    for (int iParticleFilter=0; iParticleFilter<nParticleFilters; iParticleFilter++ )
      ...
      HistogramTask.addGroupInSet(0,histos);
      }
    ...
    for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++ )
      ...
      for (int iParticleFilter2=0; iParticleFilter2<nParticleFilters; iParticleFilter2++ )
        ...
        histos->createHistograms();
        HistogramTask.addGroupInSet(1,histos);
        }
      }
    }
    ...
    for (unsigned int iParticle1=0; iParticle1<nParticles; iParticle1++)
      ...
      for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++ )
        ...
        if (particleFilters[iParticleFilter1]->accept(particle1))
          ...
          ParticleSingleHistos * histos = (ParticleSingleHistos *)  HistogramTask.getGroup(0,index);
        ...
        for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++ )
          ...
          for (int iParticleFilter2=0; iParticleFilter2<nParticleFilters; iParticleFilter2++ )
            ...
            if (accept1 & accept2)
              ...
              ParticlePairHistos * histos = (ParticlePairHistos *)  HistogramTask.getGroup(1,index);
  ...
  for (int iEventFilter=0; iEventFilter<nEventFilters; iEventFilter++ )
    ...
    if (nAccepted>1)
      ...
      for (int iParticleFilter=0; iParticleFilter<nParticleFilters; iParticleFilter++ )
        ...
        HistogramTask.getGroup(0,index)->scale(scalingFactor);
        }
      for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++ )
        {
        for (int iParticleFilter2=0; iParticleFilter2<nParticleFilters; iParticleFilter2++ )
          ...
          HistogramTask.getGroup(1,index)->scale(scalingFactor);
          }
        }
      }
    else
...
void IdentityAnalyzer::createDerivedHistograms()
{
  ...
  for (int iEventFilter=0; iEventFilter<nEventFilters; iEventFilter++ )
    ...
    for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++ )
      ...
      histos->createHistograms();
      HistogramTask.addGroupInSet(2,histos);
      }
    // pairs
    for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++ )
      {
      ...
      for (int iParticleFilter2=0; iParticleFilter2<nParticleFilters; iParticleFilter2++ )
        ...
        histos->createHistograms();
        HistogramTask.addGroupInSet(3,histos);
        }
      }
    }
  if (reportEnd(__FUNCTION__))
...
void IdentityAnalyzer::calculateDerivedHistograms()
{
  ...
  for (int iEventFilter=0; iEventFilter<nEventFilters; iEventFilter++ )
    ...
    for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++)
      ...
      bSingleHistos1 = (ParticleSingleHistos *) HistogramTask.getGroup(0,index);
      //if (reportDebug(__FUNCTION__))   cout << " (1a) iParticleFilter1:" << iParticleFilter1 << " named " << pfn1 << " with index:" << index << endl;
      dSingleHistos1 = (ParticleSingleDerivedHistos *) HistogramTask.getGroup(2,index);
    ...
    for (int iParticleFilter1=0; iParticleFilter1<nParticleFilters; iParticleFilter1++)
      ...
      for (int iParticleFilter2=0; iParticleFilter2<nParticleFilters; iParticleFilter2++)
        ...
        bSingleHistos2 = (ParticleSingleHistos *) HistogramTask.getGroup(0,index);
        dSingleHistos2 = (ParticleSingleDerivedHistos *) HistogramTask.getGroup(2,index);
        index = basePair+iParticleFilter1*nParticleFilters+iParticleFilter2;
        bPairHistos = (ParticlePairHistos *) HistogramTask.getGroup(1,index);
        dPairHistos = (ParticlePairDerivedHistos *) HistogramTask.getGroup(3,index);
