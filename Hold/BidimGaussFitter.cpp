...
pointOption(nullptr)
{
  appendClassName("GaussFitter");
  setName("GaussFitter");
  setTitle("Gauss Fitter");
  setVersion("1.0");
  setDescription("Gauss Fitting");
  initialize();
}
                                             ...
                                             const Configuration & label)
{
  ...
  // save/cache the flow properties
  ...
  {
  addProperty(f, 0, 11, 0.0); f->addProperty(iPar,flowParsCache[iPar]);
  String suffix = "_a";
  suffix += iPar;
  suffix += "_";
  ...
  // restore flow properties from local cache
  for (int iPar=0; iPar<11; iPar++) flowFitFct->addProperty(first+iPar,flowParsCache[iPar]);
...
//    }
  ...
  if (isFlow)
    ...
    for (int iPar=0; iPar<nFlowPar; iPar++)
      ...
      if (true)
        {
        // fixing flow properties
        fullFitFct->addProperty(5+iPar, value);
        ...
        if (iPar>=nFlowParUsed) // if not using eta dependent coefficients.
          {
          fullFitFct->addProperty(5+iPar, 0.0);
          fullFitFct->FixParameter(5+iPar, 0.0);
          }
        }
      }
    ...
    fullFitFct->addProperty(5, 0.0);  // flow is now removed, so is the offset
...
//  for (int iPar=0; iPar<nFlowPar;iPar++)
//  {
//  flowFitFct->addProperty(iPar, fullFitFct->GetParameter(iPar+5));
//  }
//  flowFitOnlyHist = cloneAndReset(dataHist, histBaseName+"_flowOnlyHist");
//  flowFitOnlyHist->GetXaxis()->SetRange(xLowBin,xHighBin);
//  flowFitOnlyHist->GetYaxis()->SetRange(yLowBin,yHighBin);
...
//  peakFitFct->addProperty(iPar, initialValue);
//  peakFitFct->SetParLimits(iPar, initialValue/2.0, 2.0*initialValue);
//  }
//  peakFitFct->addProperty(5,0.00001);
...
//      fullFitFct->addProperty(iPar,-1.0E-3);
...
// =========================================================================================================
// Use this function to set the name of the properties of full fit function
...
void BidimGaussFitter::setFunctionParameterNames(TF2* f, const BidimGaussFitconst Configuration & fitConfig)
{
  ...
  f->addProperty(iPar,fitConfig.initialValues [first+iPar]);
  }
}
...
// ====================================================================================
// Use this function to fix the user properties to values selected by the user.
// ====================================================================================
void BidimGaussFitter::fixUserParameters(TF1 *f, const BidimGaussFitconst Configuration & fitConfig)
{
  ...
  if (fitConfig.fixedParameters[first+iPar])
    {
    f->addProperty(iPar,fitConfig.fixedValues[first+iPar]);
...
// ==============================================================================================
// Use this function to set and fix the peak fit properties when using the full fit function.
// ==============================================================================================
void BidimGaussFitter::fixPeakParameters(TF2* f, double a, double sigmaEta, double sigmaPhi, double gammaEta, double gammaPhi)
{
  int iPar = 0;
  f->addProperty(iPar,a);        f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,sigmaEta); f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,sigmaPhi); f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,gammaEta); f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,gammaPhi); f->FixParameter(iPar, f->GetParameter(iPar));
}

// ==============================================================================================
// Use this function to set and fix the flow fit properties when using the full fit function.
// ==============================================================================================
...
{
  int iPar = 5;
  f->addProperty(iPar,a0);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a1);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a2);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a3);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a4);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a5);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a6);     f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a2eta);  f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a3eta);  f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a2eta2); f->FixParameter(iPar, f->GetParameter(iPar)); iPar++;
  f->addProperty(iPar,a3eta2); f->FixParameter(iPar, f->GetParameter(iPar));
}
...
void BidimGaussFitter::initializeFlowFitParameters(TF2 * f, const BidimGaussFitconst Configuration & fitConfig, double scale)
{
  ...
  switch (nPar)
    {
      ...
      case 11: first = 0; break;         // flow properties span 0 to 9 in this this function
      case 16: first = 5; break;         // flow properties span 5 to 14 in this this function
      default: return; break;
    }
  ...
  if (fitConfig.fixedParameters[5+iPar])
    {
    f->addProperty(first+iPar, fitConfig.fixedValues[5+iPar]);
    f->FixParameter(first+iPar, fitConfig.fixedValues[5+iPar]);
    }
  else
    {
    f->addProperty(first+iPar, fitConfig.initialValues[5+iPar]);
...
{
  int nPar  = f->GetNpar();
  if (nPar==11)  return; // Do NOT want to initialize flow properties here..
  int first = 0;
  int last  = 5;
  ...
  if (fitConfig.fixedParameters[iPar])
    {
    f->addProperty(iPar, fitConfig.fixedValues[iPar]);
    f->FixParameter(iPar, fitConfig.fixedParameters[iPar]);
    }
  else
    {
    f->addProperty(iPar, fitConfig.initialValues[iPar]);
  ...
  switch (nPar)
    {
      ...
      case 11: first = 5; break; // flow properties span 0 to 9 in this this function
      case 16: first = 5; break; // flow properties span 5 to 14 in this this function
...
void BidimGaussFitter::addProperty(TF2* f, int first, int last, double value)
{
  for (int iPar=first; iPar<=last; iPar++)
  {
  f->addProperty(iPar,value);
  }
}
...
// ==============================================================================================
// Use this function to set and fix all flow fit properties to zero when using the
// full fit function.
// ==============================================================================================
