  ...
  for (int iTemp=0; iTemp<nChemicalTemp; iTemp++ )
    ...
    for (int iMuB=0; iMuB<nMuB; iMuB++ )
      ...
      for (int iMuS=0; iMuS<nMuS; iMuS++ )
        ...
        gas->initialize();
        gas->execute();
        ThermalGasVsTempHistograms * hgVsTHistos = (ThermalGasVsTempHistograms*) HistogramTask.getGroup(1,0);
        hgVsTHistos->fill(*gas);
        ThermalGasHistograms * hgHistos = (ThermalGasHistograms*) HistogramTask.getGroup(0,iMuS);
        hgHistos->fill(*gas);
        count++;
...
void ThermalGasModel::createHistograms()
{
  if (reportStart(__FUNCTION__))
    ;
  HistogramTask.addSet("ThermalGasHistograms");
  ...
  if (doTempDependentHistos)
    ...
    histos->createHistograms();
    HistogramTask.addSet("ThermalGasVsTempHistograms");
