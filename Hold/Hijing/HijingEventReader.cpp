...
void HijingEventReader::execute()
{
  ...
  nBytes += nb;
  if (nParticles > nTracksMax)
    {
    if (reportError(__FUNCTION__))
      cout<< "nParticles: " << nParticles << "  exceeds capacity " << nTracksMax << endl;
    throw TaskException("nParticles > nTracksMax","HijingEventReader::execute()");
    }
...
void HijingEventReader::initialize()
{
